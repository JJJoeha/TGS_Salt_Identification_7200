...
case class saltImage(id:String, rle_mask:Mask)


case class Mask(v:String) {
  def apply(v:String):Mask = v match {
    case "" => null
    case v:String => if(v.length%2==0) apply(v) else throw new Exception("Invalid length of Mask string. should be even")
    case _ => throw new Exception("Error parsing Mask string")
  }
  ...
  //  lazy val matrix = Try(Mask.toMatrix(pixels))
