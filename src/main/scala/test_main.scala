...
object test_main extends App {
  protected val logger:Logger = Logger.getLogger(test_main.getClass)
...
//  val path = new File(System.getProperty("user.dir"), "Dataset/train/images")
...
//  val instanceNum :Int = Option(path.list).map(_.filter(_.endsWith(".png")).size).getOrElse(0)
  val numImages:Int = path.listFiles().filter(_.isDirectory).map(_.list).foldLeft(0)((totalCount, curDir)=>totalCount+curDir.filter(_.endsWith(".jpg")).size)
